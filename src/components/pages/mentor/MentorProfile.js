...
export default function MentorProfile() {
  ...
  return (
    <Box sx={{ p: 4, maxWidth: 700, margin: "auto", mt: 4 }}>
      <Typography variant="h4" fontWeight={700} mb={2}>Mentor Profile & Dashboard</Typography>
      <Paper sx={{ p: 3, mb: 3 }}>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Typography variant="h6" mb={1}>Availability Calendar</Typography>
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <DateCalendar value={selectedDate} onChange={handleDateSelect} />
        </LocalizationProvider>
        <Box mt={2}>
          <Typography variant="body2">Selected Dates:</Typography>
          {availability.map((date, idx) => <Chip key={idx} label={date} sx={{ mr: 1, mb: 1 }} />)}
        </Box>
      </Paper>
    </Box>
  );
